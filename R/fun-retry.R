...
#'
#' @return \code{retry} runs \code{foo} and returns the same as \code{foo}
...
#'
#' @describeIn retry
#' runs \code{foo} untill succeeds or limit reached
#'
retry <- function(foo, max.iter, fail = NA, verbose = FALSE) {
...
#' @describeIn retry
#' returns modified \code{foo} that retries itself untill succeeds ro limit reached
#'
...
#' table(replicate(10, retry(f, 2, "failed", FALSE)))
#'
#' \dontrun{
